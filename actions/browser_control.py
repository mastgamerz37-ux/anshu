...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".ansh_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".ansh_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using ANSH profile")
                ansh = str(Path.home() / ".ansh_profiles" / "firefox_ansh")
                Path(ansh).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(ansh, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".ansh_profiles" / "safari")
        ...
        # ANSH otomasyon profiline geçilir — buraya bir kez giriş yapılan
        # hesaplar sonraki oturumlarda da açık kalır.
        ansh_profile = str(Path.home() / ".ansh_profiles" / self.browser_name)
        Path(ansh_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with ANSH profile: {ansh_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(ansh_profile, **kwargs)
            self._page = await self._adopt_page()
            print(f"[Browser] ✅ Launched [{label}] with ANSH profile "
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "ansh_screenshot.png")
